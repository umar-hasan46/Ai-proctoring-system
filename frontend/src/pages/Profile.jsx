  ...
  if (!email) {
    return (
      <div className="card" style={{ maxWidth: '600px', margin: '100px auto', textAlign: 'center', padding: '50px' }}>
        ...
        <p style={{ color: '#718096' }}>Please login again to view your profile.</p>
  ...
  if (loading) {
    return (
      <div className="card" style={{ maxWidth: '800px', margin: '100px auto', textAlign: 'center', padding: '50px' }}>
        ...
        <h3 style={{ color: '#1e3a5f', marginTop: '20px' }}>Loading profile...</h3>
        <p style={{ color: '#718096' }}>Synchronizing your secure data.</p>
      </div>
    );
  }
  ...
  if (!profile) {
    return (
      <div className="card" style={{ maxWidth: '800px', margin: '40px auto', textAlign: 'center', padding: '50px' }}>
        ...
        <h2 style={{ color: '#1e3a5f' }}>Profile Not Found</h2>
        <p style={{ color: '#718096' }}>Profile information is not available yet.</p>
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '0 20px' }}>
      <div className="card" style={{ padding: '40px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', borderRadius: '20px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '150px', background: 'linear-gradient(135deg, #1e3a5f 0%, #3182ce 100%)', zIndex: 1 }}></div>
        ...
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', marginTop: '50px' }}>
          ...
          <h1 style={{ color: '#1e3a5f', marginTop: '20px', marginBottom: '5px', fontSize: '2.5rem' }}>{profile.name || 'N/A'}</h1>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', alignItems: 'center' }}>
            ...
            <span style={{ color: '#718096', fontSize: '0.9rem' }}>Member since {profile.created_at_ist || 'N/A'}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginTop: '3rem' }}>
          ...
          <div className="card" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
            <h3 style={{ color: '#1e3a5f', marginTop: 0, borderBottom: '2px solid #cbd5e0', paddingBottom: '10px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              📧 Contact Details
            </h3>
            ...
            {profile.role !== 'Admin' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontWeight: '800', color: '#1e3a5f', fontSize: '1.8rem' }}>{profile.latest_score || 'N/A'}</div>
                </div>
              </div>
            ) : (
              <div style={{ padding: '10px' }}>
                <p style={{ color: '#4a5568', margin: '5px 0' }}>✅ System Administrator</p>
                <p style={{ color: '#4a5568', margin: '5px 0' }}>✅ Live Proctoring Access</p>
                <p style={{ color: '#4a5568', margin: '5px 0' }}>✅ User Performance Oversight</p>
              </div>
            )}
          </div>
        </div>
